...
const SECTIONS = [
  {
    title: "문학",
    items: ["소설/시/희곡", "라이트 노벨", "판타지/무협", "에세이", "고전", "로맨스"],
  },
  {
    title: "인문사회",
    items: ["인문/사회", "언어학", "역사", "인물/평전", "종교/역학", "사회과학", "법률", "외국어", "전기/자서전"],
  },
  ...
  {
    title: "생활/가족",
    items: ["가정/요리/뷰티", "건강/취미/레저", "여행", "육아", "어린이", "유머", "요리", "가정/원예/인테리어", "청소년", "좋은부모", "건강/스포츠", "가족/관계"],
  },
  { title: "만화", items: ["만화"] },
] as const;
    ...
    <div className="w-full flex flex-col gap-8">
      {SECTIONS.map((sec) => (
        <div key={sec.title} className="flex flex-col gap-2">
          ...
          {/* (5) 설명 문구 */}
          <span className="text-[#697198] text-[14px] font-medium leading-5.25 pb-2">
            복수 선택이 가능합니다.
          </span>
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    selected ? "bg-[#7AD8D2]" : "bg-[#1B203B]",
                  ].join(" ")}
                >
                  <span
                    className={[
                      "text-[14px] font-semibold leading-[14px]",
                      selected ? "text-[#13172A]" : "text-[#8B94B2]",
                    ].join(" ")}
                  >
                    {item}
                  </span>
